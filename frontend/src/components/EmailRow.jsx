  ...
  return (
    <li className="flex items-center justify-between gap-4 px-4 py-3">
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium text-brand-text">{email.subject}</p>
        <p className="truncate text-xs text-brand-text/70">{email.sender}</p>
      </div>

      <div className="flex shrink-0 items-center gap-3">
        {!wasCorrected && email.category_confidence != null && (
          <span className="text-xs text-brand-text/60">{Math.round(email.category_confidence * 100)}%</span>
        )}
        <select
          ...
          className={`rounded-md border px-2 py-1 text-xs ${
            wasCorrected
              ? 'border-brand-amber bg-brand-amber text-brand-plum'
              : 'border-brand-text/25 bg-brand-plum/70 text-brand-text'
          }`}
        >
          {CATEGORIES.map((cat) => (
        ...
        <span className="w-20 shrink-0 text-right text-xs text-brand-text/60">
